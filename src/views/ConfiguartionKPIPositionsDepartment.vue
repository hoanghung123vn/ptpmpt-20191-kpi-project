...
export default {
  ...
  mounted() {
    bus.$on("addKPI", department => {
      ...
      alert(
        "Bạn đã thêm mới tiêu chí KPI. Hãy cập nhật lưới tiêu chí KPI bên dưới"
      );
      //this.$swal("Great!", "Tạo mới thành công", "success");
    });
    bus.$on("deleteKPI", name => {
      this.dataKPI.criterias = this.dataKPI.criterias.filter(
    ...
    bus.$on("updateKPI", department => {
      const index = this.dataKPI.criterias.findIndex(
        kpi => kpi.name === department.name
      );
      alert(
        "Bạn đã thay đổi tiêu chí KPI. Hãy cập nhật lưới tiêu chí KPI bên dưới"
      );
  ...
  methods: {
    ...
    async DepartmentChange() {
      ...
      const found = this.nameDepartment.findIndex(
        element => element === this.DepartmentId
      );
      const idDepartment = this.listDepartment[found].id;
      this.idDepartment = idDepartment;
      if (idDepartment > 0) {
        kpiPositions.style.display = "block";
        var responsePositions = await configurationKPIService.getlistEmployeeByDepartmentID(
          idDepartment
        );
    ...
    async PositionsChange() {
      ...
      const found = this.namePositions.findIndex(
        element => element === this.PositionsId
      );
